...
const katexInline = (tex, options) => {
    ...
    try {
        ...
        if (options.throwOnError)
            console.warn(error);
        return `<span class='katex-error' title='${utils_1.escapeHtml(error.toString())}'>${utils_1.escapeHtml(tex)}</span>`;
    }
};
const katexBlock = (tex, options) => {
    ...
    try {
        ...
        if (options.throwOnError)
            console.warn(error);
        return `<p class='katex-block katex-error' title='${utils_1.escapeHtml(error.toString())}'>${utils_1.escapeHtml(tex)}</p>`;
    }
};
