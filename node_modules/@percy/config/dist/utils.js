...
function joinPropertyPath(path) {
  return path.map(part => isArrayKey(part) ? `[${part}]` : `.${part}`).join('').substr(1);
...
function merge(sources, map) {
  return sources.reduce((target, source, i) => {
    ...
    walk(source, (path, value) => {
      ...
      let prev = ctx === null || ctx === void 0 ? void 0 : ctx[key]; // maybe map the property path and/or value

      let [p, next] = (map === null || map === void 0 ? void 0 : map(path, prev, value)) || [];
      if (p) path = [...p]; // adjust path to concat array values when necessary

      if (next !== null && (isArray(ctx) || isInteger(key))) {
        var _ctx$length;

        path.splice(-1, 1, (_ctx$length = ctx === null || ctx === void 0 ? void 0 : ctx.length) !== null && _ctx$length !== void 0 ? _ctx$length : 0);
      } // delete prev values
